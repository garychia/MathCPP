...
InputType HingeLossGradient(const InputType &input, const LabelType &prediction,
...
Vector<double> Perceptron(const Matrix<DataType> &data,
...
*/
template <class DataType, class LabelType>
Vector<double> AveragedPerceptron(const Matrix<DataType> &data,
                                  const Vector<LabelType> &labels,
                                  std::size_t T = 1000);
...
InputType GradientDescent(const std::function<OutputType(const InputType &)> &f,
                          ...
                          std::size_t iterations, bool recordHistory = false,
