...
NeuralNetwork::NeuralNetwork(const List<LayerType> &layerTypes,
                             const List<Tuple<unsigned int>> &shapes,
                             LossType lossType)
    : layers() {
  if (layerTypes.Size() != shapes.Size())
    throw Exceptions::InvalidArgument("NeuralNetwork: Input and output size "
  ...
  for (std::size_t i = 0; i < layerTypes.Size(); i++) {
    ...
    case LayerType::Linear: {
      if (shapes[i].Size() != 2)
        throw Exceptions::InvalidArgument(
            "NeuralNetwork: the input and output sizes of a LinearLayer must "
            "be specified.");
    ...
    case LayerType::BatchNormalization: {
      if (shapes[i].Size() != 1)
        throw Exceptions::InvalidArgument(
            "NeuralNetwork: the size of input to a BatchNormLayer must be "
            "specified.");
...
List<double> NeuralNetwork::Train(const Matrix<double> &trainingData,
...
std::ostream &operator<<(std::ostream &stream,
                         const NeuralNetwork::LayerType &layerType) {
  stream << static_cast<int>(layerType);
  return stream;
}

std::ostream &operator<<(std::ostream &stream,
                         const NeuralNetwork::LossType &lossType) {
